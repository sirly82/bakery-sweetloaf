...
$nama_user = $_SESSION['name'] ?? ''; // Nilai default string kosong jika null
$telepon_user = $user_data['phone'] ?? '';
$alamat_user = $user_data['address'] ?? '';

// Ambil data profil pengguna untuk mengisi form pengiriman
$stmt_user = $conn->prepare("SELECT phone, address FROM users WHERE id = ?");
...
$sql_cart = "
    SELECT 
        k.produk_id,  -- ← tambahkan baris ini
        k.jumlah, 
...
$cart_items[] = [
    'produk_id' => $row['produk_id'], // ← penting!
...
<body>
    ...
    <button type="submit" class="btn-proceed-payment">Lanjutkan ke Pembayaran</button>
</div>
    ...
    <script src="order.js"></script> </body>
</html>